...
-(void)removeAll
{
    ...
    NSLog(@"走魅族。。。。。。。。。。。。。IIIIIIIIIIIIII    OOOOOOOOOOOOO  PPPPPPPPPPPPPPP");
    ...
    if ([self.userGigtInfos objectForKey:userID]) {
        ...
        if ([userID integerValue] % 2) {
            ...
            if (op.model.giftCount != 0) {
                op.presentView.frame  = CGRectMake(-self.parentView.frame.size.width / 2, 300, self.parentView.frame.size.width / 1.5, 40);
            ...
            if (op.model.giftCount != 0) {
                op.presentView.frame  = CGRectMake(-self.parentView.frame.size.width / 2, 240, self.parentView.frame.size.width / 1.5, 40);
                op.presentView.originFrame = op.presentView.frame;
