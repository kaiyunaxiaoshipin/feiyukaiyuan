...
-(id)initWithFrame:(CGRect)frame textArray:(NSArray *)textArray font:(UIFont *)font textColor:(UIColor *)textColor{
    if(self = [super initWithFrame:frame]){
        _textArray = textArray;
        _font = [UIFont fontWithName:@"Arial-ItalicMT" size:18];;
...
-(void)commonInit{
    ...
    if(_textArray.count == 1){
        ...
        if(rect.size.width > KSelfWith){
            ...
            _CanRolling = NO;
            mylabels *label = [[mylabels alloc] initWithFrame:
    ...
    if(_CanRolling){
        ...
        for (int i = 0; i < kNumberOfLabel; i++) {
            ...
            _labels[i].font = [UIFont fontWithName:@"Arial-ItalicMT" size:18];
            _labels[i].shadowColor = [UIColor blackColor];
            _labels[i].shadowOffset = CGSizeMake(0,0.5);
