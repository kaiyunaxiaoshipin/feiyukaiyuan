...
-(void)navtion{
    ...
    UISegmentedControl *segment = [[UISegmentedControl alloc]initWithItems:@[@"坐骑",@"靓号"]];
...
-(void)change:(UISegmentedControl *)segment{
    ...
    if (segment.selectedSegmentIndex == 0) {
        [wevView stopLoading];
        //坐骑
        NSString *paths = [h5url stringByAppendingFormat:@"/index.php?g=Appapi&m=Car&a=mycar&uid=%@&token=%@",[Config getOwnID],[Config getOwnToken]];
    ...
    //靓号
        ...
        [wevView stopLoading];
        NSString *paths = [h5url stringByAppendingFormat:@"/index.php?g=Appapi&m=Liang&a=myliang&uid=%@&token=%@",[Config getOwnID],[Config getOwnToken]];
        url = [NSURL URLWithString:paths];
        NSURLRequest *request = [NSURLRequest requestWithURL:url];
        [wevView removeFromSuperview];
        wevView = nil;
