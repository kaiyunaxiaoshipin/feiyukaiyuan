...
@implementation TiStickerManager
{
...
+ (instancetype)sharedManager {
    ...
    dispatch_once(&onceToken, ^{
        _stickersManager = [TiStickerManager new];
    });
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _ioQueue = dispatch_queue_create("com.tillusory.stickers", DISPATCH_QUEUE_SERIAL);
        _stickerPath = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"sticker"];
...
- (BOOL)getStickersFromJson {
    ...
    //拷贝本地贴纸到沙盒
    NSString *localPath =
            [[[NSBundle mainBundle] pathForResource:@"TiResource" ofType:@"bundle"] stringByAppendingPathComponent:@"sticker"];
...
//    
    ...
    for (NSDictionary *itemDict in newArr) {
        ...
        TiSticker *sticker =
                [[TiSticker alloc] initWithName:[itemDict valueForKey:@"name"] thumbName:[itemDict valueForKey:@"thumb"] download:[[itemDict valueForKey:@"downloaded"] boolValue] DirectoryURL:url];
        if ([[itemDict valueForKey:@"sourceType"] intValue] == 0) {
            sticker.sourceType = TiStickerSourceTypeFromTi;
        } else {
            sticker.sourceType = TiStickerSourceTypeFromLocal;
        }
