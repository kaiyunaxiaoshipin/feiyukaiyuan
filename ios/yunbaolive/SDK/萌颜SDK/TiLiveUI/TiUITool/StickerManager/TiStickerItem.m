...
- (instancetype)initWithName:(NSString *)name thumbName:(NSString *)thumb download:(BOOL)download DirectoryURL:(NSURL *)dirurl {
    if (self = [super init]) {
        ...
        _downloadState = TiStickerDownloadStateDownoadNot;
        _stickerDir = nil;
        _stickerSound = nil;
        _items = nil;
        ...
        if (download == YES) {
            ...
            _stickerDir = dir;
            _downloadState = TiStickerDownloadStateDownoadDone;
            ...
            TiStickerItem *itemCopy;
            for (NSDictionary *itemDict in itemsDict) {
                TiStickerItem *item = [[TiStickerItem alloc] initWithJSONDictionary:itemDict];
...
- (void)setSourceType:(TiStickerSourceType)sourceType {
    _sourceType = sourceType;
    if (sourceType == TiStickerSourceTypeFromTi) {
        // if you want to use Ti's downloadURL, you donot need to do anything
        _downloadURL = nil;
    } else if (sourceType == TiStickerSourceTypeFromLocal) {
...
- (void)setPlayCount:(NSUInteger)count {
    ...
    for (TiStickerItem *item in _items) {
...
+ (void)resetSticker:(TiSticker *)sticker {
