...
class DetailController extends HomebaseController {
	...
	function index(){       
		...
		$Gift=M("gift");
		$list=$Coinrecird->field("uid,giftid,sum(giftcount) as giftcounts,sum(totalcoin) as total")->where("action='sendgift' and touid={$uid}")->group("uid,giftid,showid")->order("addtime desc")->limit(0,50)->select();
		foreach($list as $k=>$v){
	...
	public function receive_more()
	{
		...
		$Coinrecird=M("users_coinrecord");
		$Gift=M("gift");
		$list=$Coinrecird->field("uid,giftid,sum(giftcount) as giftcounts,sum(totalcoin) as total")->where("action='sendgift' and touid={$uid}")->group("uid,giftid,showid")->order("addtime desc")->limit($start,$pnums)->select();
		foreach($list as $k=>$v){
			$giftinfo=$Gift->field("giftname")->where("id={$v['giftid']}")->find();
			if(!$giftinfo){
				$giftinfo=array(
