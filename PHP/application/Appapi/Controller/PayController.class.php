...
class PayController extends HomebaseController {
	...
	*/
	...
	public function notify_ios(){
		...
		//判断订单是否存在
		$orderinfo=M("users_charge")->where("orderno='{$out_trade_no}' and coin='{$chargeinfo['coin']}' and status='0' and type='3'")->find();
	...
	/* 码支付 */
	public function erweimapay()
	{
		$uid=I("uid");
		$token=I("token");
		$configpub = getconfigpri();
		/*if(checkToken($uid,$token)==700){
			$this->assign("reason",'您的登陆状态失效，请重新登陆！');
			$this->display(':error');
			exit;
		} */
		$this->assign("configpri",$configpub);
		$this->assign("uid",$uid);
		$this->display();		
	}
