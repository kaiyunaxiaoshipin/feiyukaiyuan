...
//		$indexLive=M("users_live")->query("select l.* from __PREFIX__users_live l left join __PREFIX__users u on l.uid=u.id where l.islive='1' and u.isrecommend='1' and l.type='0' ");
        $indexLive=M("users_live l")
            ->field("u.user_nicename,u.avatar,l.thumb,l.uid,l.isvideo,l.pull,l.stream,l.type,l.islive")
            ->join("left join {$prefix}users u on u.id=l.uid")
            ->where("l.islive='1' and u.isrecommend='1' and l.type='0' ")
            ->limit(4)
            ->select();
		//var_dump($indexLive);
		...
		$recommend=M("users_live l")
					->field("u.user_nicename,u.avatar,l.thumb,l.uid,l.stream,l.type,l.islive")
					->join("left join {$prefix}users u on u.id=l.uid")
					->where("l.islive='1'")
					->limit(12)
					->select();
		...
		/* 热门（在直播，推荐为热门） */
		$hot=M("users_live l")
					->field("u.user_nicename,u.avatar,l.uid,l.thumb,l.stream,l.title,l.city,l.islive,l.type,u.signature")
					->join("left join {$prefix}users u on u.id=l.uid")
		...
		/* 最新直播（在直播，按开播时间倒序） */ 
		$live=M("users_live l")
            ->field("l.uid,u.avatar,u.user_nicename,l.thumb,l.stream,l.title,l.city,l.islive,l.type")
            ->join("left join {$prefix}users u on u.id=l.uid")
            ->where("islive='1'")
