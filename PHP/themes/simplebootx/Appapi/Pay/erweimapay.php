...
function getConfigPri()
{
    ...
    if (!$config) {
        $config = DI()->notorm->options->select('option_value')->where("option_name='configpri'")->fetchOne();
...
print_r($_SESSION['uid']);// 打印全部SESSION  需要先开启session
//print_r($_COOKIE);// 打印全部cookie


//trim($_GET['id'])
$username = $_SESSION['uid']; //   http://域名/codepay/?user=admin 访问则自动填写了账户信息
