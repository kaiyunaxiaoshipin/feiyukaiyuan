				...
				if(res != null){
					...
					if(userInfo['id'] == data.uid ){
                        ...
                        if(socket.roomnum==socket.uid){
                            clientRedis.hget('LiveConnect',socket.uid,function(error,res){
                                if(error){
                                    return;
                                }
                                if(res){
                                    ...
                                    clientRedis.hget('LivePK',socket.uid,function(error,res){
                                        if(error){
                                            return;
                                        }
                                        if(res){
                                    ...
                                    clientRedis.hget('LivePK_gift',socket.uid,function(error,res){
                                        if(error){
                                            return;
                                        }
                                        if(res){
                                            LivePK_gift[socket.uid]=res;
                                        }
                                    })
                                }
                            })
