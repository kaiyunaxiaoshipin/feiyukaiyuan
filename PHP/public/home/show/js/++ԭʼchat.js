		...
		$.getJSON("/index.php?g=home&m=Spend&a=roomCharge", {liveuid:liveuid,stream:stream,type:type},
			...
			{  
				...
				else
				{
					...
					if(type_val){
						var msg = '{"retcode":"000000","retmsg":"ok","msg":[{"_method_":"updateVotes","action":"1","msgtype":"26","votes":"'+type_val+'","uid":"'+_DATA.user.id+'","isfirst":"0","ct":"","vip_type":"0","liangname":"0"}]}';
						Socket.emitData('broadcast',msg);
					}
...
var Controls={
	...
	cancel:function()//设置 取消 管理
	{
      ...
      success: function(data)
      {
				...
				var msg='{"msg":[{"_method_":"SystemNot","action":"13","ct":"'+ct+'","msgtype":"4","uname":"'+_DATA.user.user_nicename+'","toname":"'+data.user_nicename+'","touid":'+touid+',"uid":'+_DATA.user.id+',"level":'+_DATA.user.level+',"vip_type":"0","liangname":"0"}],"retcode":"000000","retmsg":"ok"}';
				Socket.emitData('broadcast',msg);
      }
    });
 		...
 		$.ajax({
			...
			success:function(data){
				if(data.code==0){
					...
					var msg = '{"msg":[{"_method_":"ShutUpUser","action":"1","ct":"'+name+'被禁言了'+data.info.shut_time+'","msgtype":"4","uid":'+_DATA.user.id+',"uname":"'+_DATA.user.user_nicename+'","touid":'+touid+',"toname":"'+name+'","vip_type":"0","liangname":"0"}],"retcode":"000000","retmsg":"ok"}';
					Socket.emitData('broadcast',msg);
				}else{
					layer.msg(data.msg);
				}
	...
	tiren:function()//踢人
	{
		...
		$.ajax({
			...
			success:function(data){
				if(data.code==0){
					layer.msg("操作成功");
				 	var msg = '{"msg":[{"_method_":"KickUser","action":"2","ct":"'+name+'被踢出房间","msgtype":"4","uid":'+_DATA.user.id+',"uname":"'+_DATA.user.user_nicename+'","touid":'+touid+',"touname":"'+name+'","vip_type":"0","liangname":"0"}],"retcode":"000000","retmsg":"ok"} ';
					Socket.emitData('broadcast',msg); 
				}else{
					layer.msg(data.msg);
				}
...
var User={
	...
	popup:function(id)
	{
        ...
        success: function(data)
        {
          ...
          html+='<div class="popup" id="popup_info" popup-data-name="'+data.info.user_nicename+'" popup-data-uid="'+data.info.id+'" style="top:'+top+'px;display:none"><div class="pop_content clearfix">';
		...
		$.ajax({
			...
			success:function(data){
				if(data.error==0){
					...
					var msg='{"msg":[{"_method_":"SystemNot","action":"13","ct":"'+ct+'","msgtype":"4","uname":"'+_DATA.user.user_nicename+'","toname":"'+_DATA.anchor.user_nicename+'","touid":'+_DATA.anchor.id+',"uid":'+_DATA.user.id+',"level":'+_DATA.user.level+',"vip_type":"0","liangname":"0"}],"retcode":"000000","retmsg":"ok"}';
				Socket.emitData('broadcast',msg);
				}else{
...
var Video={
	...
	setHeight: function(){
		...
		this.LF_video.css({"width":w,"height":height});
		this.player.css({"width":w,"height":height});
	...
	endRecommend: function(){
		...
		$(".channel-player-v2").css('display','none');
		$.ajax({
			url:_this.endReUrl,
			data:{},
			dataType:'json',
	...
	sendChat: function(){
      ...
      success: function(data)
      {
				...
				else
				{
					if(data['admin']==60)
					{
						var msg='{"msg":[{"_method_":"SystemNot","action":"13","ct":"'+chatmsg+'","msgtype":"4","uname":"'+_DATA.user.user_nicename+'","toname":"'+_DATA.anchor.user_nicename+'","touid":'+_DATA.anchor.id+',"uid":'+_DATA.user.id+',"level":'+_DATA.user.level+',"vip_type":"'+ _DATA.user.vip.type +'","liangname":"'+ _DATA.user.liang.name+'"}],"retcode":"000000","retmsg":"ok"}';
					}
					else
					{
						var msg = '{"msg":[{"_method_":"SendMsg","action":0,"ct":"'+chatmsg+'","msgtype":"2","tougood":"","touid":"","touname":"","ugood":"'+_DATA.user.id+'","uid":"'+_DATA.user.id+'","uname":"'+_DATA.user.user_nicename+'","level":"'+_DATA.user.level+'","vip_type":"'+ _DATA.user.vip.type +'","liangname":"'+ _DATA.user.liang.name+'"}],"retcode":"000000","retmsg":"OK"}';
					}
					Socket.emitData('broadcast',msg);		
				}
      }
		...
		$.ajax({
			...
			success:function(data){
				...
				if(data.errno==700){
					...
					var msg = '{"retcode":"000000","retmsg":"ok","msg":[{"_method_":"SendGift","evensend":"'+data.evensend+'","action":"0","ct":"'+data.gifttoken+'","msgtype":"1","level":"'+data.level+'","uid":"'+data.uid+'","timestamp":"'+WlTools.FormatNowDate()+'","uname":"'+_DATA.user.user_nicename+'","uhead":"'+_DATA.user.avatar+'","vip_type":"'+ _DATA.user.vip.type +'","liangname":"'+ _DATA.user.liang.name+'"}]}'; 
					Socket.emitData('broadcast',msg);
				}
			},
			error: function(data)
...
var Light={
	...
	light:function()
	{
		...
		{
			 _DATA.user.light=1;
			var msg = '{"retcode":"000000","retmsg":"ok","msg":[{"_method_":"SendMsg","action":"0","msgtype":"2","level":"'+_DATA.user.level+'","uname":"'+_DATA.user.user_nicename+'","uid":"'+_DATA.user.id+'","heart":"1","ct":"我点亮了","vip_type":"'+ _DATA.user.vip.type +'","liangname":"'+ _DATA.user.liang.name+'"}]}';
			Socket.emitData('broadcast',msg);
		}
		/* if(time<=timestamp)
		{	
		...
		var light = '{"msg":[{"_method_":"light","action":"2","msgtype":"0","vip_type":"0","liangname":"0"}],"retcode":"000000","retmsg":"OK"}';
